...
public class WidgetItemClickIntentReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intentI) {
        ...
        Util.createAndSendAnalyticsData(Util.CLICKED_APP, Util.CATEGORY_MANUAL, Util.APP_PACKAGE_NAME, extraPackageName, mContext);
        if(extra1.equals("")){
            ...
            try {
                ...
                if(set.contains(extraPackageName)){
                  //  new SendAnalyticsData(mContext, Util.APP_ENGAGEMENT).execute(extraPackageName);
                    Util.createAndSendAnalyticsData(Util.APP_ENGAGEMENT, Util.CATEGORY_MANUAL, Util.APP_PACKAGE_NAME, extraPackageName, mContext);
                }else
                {
                  ...
                  //  new SendAnalyticsData(mContext, Util.ACTIVATED_APP).execute(extraPackageName);
                    Util.createAndSendAnalyticsData(Util.ACTIVATED_APP, Util.CATEGORY_MANUAL, Util.APP_PACKAGE_NAME, extraPackageName, mContext);
                }
            } catch (Exception e) {
